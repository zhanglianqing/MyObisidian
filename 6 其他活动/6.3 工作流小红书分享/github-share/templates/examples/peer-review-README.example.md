...
> 期刊邀稿、在审、二轮修订的**工作区**。任务总览 → `00_ReviewTrack`；下一节点 → `同期项目-下一节点#10 · 杂志审稿`。  
> **复制到你的 vault**：另存为 `10 杂志审稿/README.md`。locate 续审时 AI 先读此页。
...
每篇稿一个子文件夹，**文件夹名以稿件号开头**（如 `JOURNAL-26-xxxx …`）。**二轮修订不新建文件夹**——仍在原目录下更新 `round` 与「二轮修订」小节（可能在 `archive/`）。
...
| 阶段 | 位置 | `status` | 动作 |
|------|------|----------|------|
| 接受邀稿 ~ 撰写中 | `active/` | `accepted` / `drafting` | 建文件夹 + 更新 `00_ReviewTrack`、下一节点 |
...
4. 更新 `00_ReviewTrack`、`archive/README`、下一节点
...
```text
维护杂志审稿：新邀稿在 active/ 建 JOURNAL-xxxxx 文件夹 + 更新 00_ReviewTrack 与同期项目-下一节点；提交 EM 后按「归档清单」移入 archive/；二轮在同一文件夹追加 round。
```
...
- 用户说「已提交 / 归档」：按上文「归档清单（提交后）」移入 `archive/` 并更新 Track。
